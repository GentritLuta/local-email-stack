# -*- coding: utf-8 -*-
"""dorian-report.py — daily client report for Mercury Scales (Dorian Skiljo), AS
DETAILED as the Aureon (daily-report.py) report, scoped to the dorian profile.
Reuses daily-report.py's render + intent helpers so all reports stay consistent.
Sections: headline KPIs, 24h/7d/30d window table, reply-intent breakdown, per-step
funnel, sender health (outreach./team.mercuryscales.com), persona head-to-head,
pipeline, recent replies, alerts.

Sends FROM "Mercury Scales Ops <reports@team.mercuryscales.com>" on the full-access
Resend account TO skiljodorian@gmail.com + info@aureonglobal.de.
Schedule: LES-dorian-report daily 08:00.

  py scripts/dorian-report.py            # send to both
  py scripts/dorian-report.py --dry      # print, no send
  py scripts/dorian-report.py --to X@y   # override recipients
"""
...
DORIAN = load_profile("dorian")
RK = DORIAN.get("relay", {}).get("resend_api_key", "")               # full-access account key
DORIAN_DOMAINS = {d["domain"].lower() for d in DORIAN.get("relay", {}).get("from_domains", [])}
DORIAN_PERSONAS = {p["slug"]: p for p in DORIAN.get("personas", [])}
RECIPIENTS = ["skiljodorian@gmail.com", "info@aureonglobal.de"]
FROM = "Mercury Scales Ops <reports@team.mercuryscales.com>"
ACCENT = "#3730a3"
...
def fetch() -> dict:
    ...
    sends = [s for s in sends if _dom(s.get("from_addr")) in DORIAN_DOMAINS]
    ...
    replies = [r for r in replies if r.get("profile_slug") == "dorian" or _dom(r.get("to_addr")) in DORIAN_DOMAINS]
    prospects = supa_get("prospects?profile_slug=eq.dorian&select=id,email,first_name,company,"
                         "verified,unsubscribed,unsubscribed_at,niche_slug&limit=10000")
    sid = supa_get("sequences?profile_slug=eq.dorian&select=id")
...
def aggregate(data: dict) -> dict:
    ...
    sub = {}
    for d in DORIAN.get("relay", {}).get("from_domains", []):
    ...
    for s in sends:
        ...
        pmeta = DORIAN_PERSONAS.get(k, {})
        row = persona.setdefault(k, {"name": pmeta.get("from_name", k), "title": pmeta.get("title", ""),
    ...
    # dorian-default personalizes on {first_name}; company is optional (personal brands).
    eligible = [p for p in verified if p["id"] not in enrolled_ids and p.get("first_name")]
...
def render(a: dict) -> str:
    P, G, R, A, M, RULE = ACCENT, dr.GREEN, dr.RED, dr.AMBER, dr.MUTED, dr.RULE
    ...
    blocks = [dr.section("Today at a glance (last 24h)", "Mercury Scales outreach snapshot since yesterday.", head)]
    ...
    blocks.append(dr.section("Sender health (rolling 7d)", "Mercury Scales senders (outreach. + team.mercuryscales.com), with warmup day.", si))
    ...
    blocks.append(dr.section("Persona head-to-head (last 30d)", "Dorian vs Mara — each identity's engagement. Diverging opens signal copy/signature differences.", pi))
    ...
    blocks.append(dr.section("Pipeline snapshot", "Self-made founder leads (YouTube/social sourcing + CSV imports) ready for the daily 09:30 enrollment.", pli))
    ...
    blocks.append(dr.section("Recent replies (last 24h)", "Every reply that landed, with snippet — positives route to calendly.com/skiljodorian/new-meeting.", ri))
...
<div style="font-size:18px;font-weight:700;color:#ffffff;letter-spacing:.4px;">Mercury Scales — Daily Outreach Report</div>
<div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1.4px;margin-top:6px;">{dt.datetime.now().strftime("%A, %B %d %Y")} &middot; client acquisition for self-made B2B founders</div>
...
<div style="font-size:11px;color:#888;">Generated by Aureon Global for Skiljo Enterprise (Mercury Scales) &middot; {dt.datetime.now().strftime("%H:%M")}</div>
...
def main() -> int:
    ...
    print("fetching Mercury Scales data..."); data = fetch()
    ...
    t = agg["today"]
    subject = (f"Mercury Scales daily: {t['sent']} sent, {dr.pct(t['opened'], t['delivered'])} open, "
    ...
    payload = {"from": FROM, "to": recipients, "subject": subject, "html": html,
               "tags": [{"name": "kind", "value": "dorian_daily_report"}]}
